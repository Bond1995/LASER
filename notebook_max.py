...
# 
res1 = []
for run in api.runs("w16", {"config.algorithm": "powersgd"}):
    try:
        res1.append(
            (
                run.id,
...
res2 = []
for run in api.runs("w16", {"config.algorithm": "sgd"}):
    try:
        res2.append(
            (
                run.id,
...
res3 = []
for run in api.runs("w16", {"config.algorithm": "sketching"}):
    try:
        res3.append(
            (
                run.id,
...
res4 = []
for run in api.runs("w16", {"config.algorithm": "randomk"}):
    try:
        res4.append(
            (
                run.id,
...
res5 = []
for run in api.runs("w16", {"config.algorithm": "signum"}):
    try:
        res5.append(
            (
                run.id,
...
ax.axhline(y = 0.93, color="black", linestyle = '--', label=" ")
...
fig.savefig("cifar10-power-accuracy.pdf")